    ...
    <section className="itemBank">
      ...
      <div className="itemBankGrid">
        ...
        <div className="itemBankEditor">
          {selectedItem ? (
            <>
              <div className="questionTopline"><h3>Редактор задания</h3><div className="actionGroup"><button className="danger" type="button" onClick={deleteSelectedItem}>Удалить</button><button className="primary" type="button" onClick={saveSelectedItem} disabled={isSaving}>{isSaving ? 'Сохраняем...' : 'Сохранить'}</button></div></div>
...
function successMessage(usedMode) {
  ...
  return 'Банк заданий сформирован шаблонным генератором. После экспертной правки сохраняйте примеры в обучающую выборку.';
}

function sourceKindLabel(value) {
  return ({ template: 'шаблон', learned_example: 'по экспертному примеру', narrow_llm: 'узкая модель ФОС' }[value] || value || 'шаблон');
}
