...
function App() {
  ...
  return (
    <main className="page">
      <section className="hero card"><div><p className="eyebrow">ВКР · MVP-прототип</p><h1>Система формирования оценочных материалов по РПД</h1><p className="heroText">Загрузите рабочую программу дисциплины, сформируйте структуру ФОС, создайте контрольные материалы и согласуйте результат.</p></div><div className="userBox"><strong>{user.full_name}</strong><span>{user.email}</span><span>Роль: {roleLabel(user.role)}</span><button className="secondary smallButton" onClick={() => logout()}>Выйти</button></div></section>
      {error && <div className="alert">{error}</div>}{success && <div className="success">{success}</div>}
      <section className="grid"><form className="card" onSubmit={uploadProgram}><h2>1. Загрузка РПД</h2><input className="fileInput" type="file" accept=".docx,.pdf,.txt" onChange={(e) => setFile(e.target.files?.[0] || null)} /><button className="primary" disabled={isUploading}>{isUploading ? 'Анализируем...' : 'Загрузить и проанализировать'}</button></form><section className="card"><h2>2. Быстрая генерация контрольной</h2><label>Вариантов<input type="number" min="1" max="20" value={params.variants_count} onChange={(e) => setParams({ ...params, variants_count: Number(e.target.value) })} /></label><label>Заданий<input type="number" min="1" max="50" value={params.questions_per_variant} onChange={(e) => setParams({ ...params, questions_per_variant: Number(e.target.value) })} /></label><label>Сложность<select value={params.difficulty} onChange={(e) => setParams({ ...params, difficulty: e.target.value })}><option value="easy">Базовый</option><option value="medium">Средний</option><option value="hard">Повышенный</option></select></label><label>Типы<input value={params.question_types.join(', ')} onChange={(e) => updateQuestionTypes(e.target.value)} /></label><button className="primary" type="button" onClick={runGeneration} disabled={isGenerating || !program}>{isGenerating ? 'Генерируем...' : 'Сформировать контрольную'}</button></section></section>
      <section className="card historyCard"><div className="sectionHeader"><div><h2>История работы</h2><p className="muted">Материалы сохраняются в SQLite.</p></div><button className="secondary" onClick={loadHistory}>{isHistoryLoading ? 'Обновляем...' : 'Обновить'}</button></div><div className="historyGrid"><HistoryList title="РПД" items={programsHistory} getKey={(x) => x.program_id} renderItem={(x) => <><strong>{x.filename}</strong><span>{x.topics.length} тем · качество анализа {x.analysis_report?.diagnostics?.quality_score || 0}%</span></>} onOpen={(x) => openProgram(x.program_id)} /><HistoryList title="Генерации" items={generationsHistory} getKey={(x) => x.session_id} renderItem={(x) => <><strong>{x.session_id.slice(0, 8)}...</strong><span>{statusLabel(x.status)} · {x.quality_report.total_questions} заданий</span></>} onOpen={(x) => openGeneration(x.session_id)} /></div></section>
